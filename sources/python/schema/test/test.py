...
class TestCreation(TestCase):
    ...
    """def test_transfo_2(self):
        
        directory = os.listdir(path)

        for files in directory:
            if files.startswith('topo2'):
                path_patch = os.path.join(path, files)
        ...
        for file in direct:
            if file.startswith('schema_V1'):
                path_patch = os.path.join(path, file)
        ...
        transfo = jsonpatch.make_patch(V1, topo2)
